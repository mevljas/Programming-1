...
def koordict(kraji):
    slovar = {}
    for kraj, kordianta1, kordinata2 in kraji:
        slovar[kraj] = (kordianta1, kordinata2)
    return slovar


def razdalje(kraj, kraji):
    mnozica = set()
    slovar_krajev = koordict(kraji)
    x, y = slovar_krajev[kraj]
    for kraj1, x2, y2 in kraji:
        if kraj != kraj1:
            mnozica.add((((x2 - x) ** 2 + (y2 - y) ** 2), kraj1))
    return mnozica


def sosedi(n, kraj, kraji):
    mnozica_razdalj = sorted(razdalje(kraj, kraji))
    najblizji_kraji = mnozica_razdalj[:n]
    najblizji_kraji = [item[1] for item in najblizji_kraji]
    return set(najblizji_kraji)


def povezave(n, kraji):
    slovar_povezav = {}
    for kraj, x, y in kraji:
        slovar_povezav[kraj] = sosedi(n, kraj, kraji)
    return slovar_povezav


def dvosmerno(d):
    d2 = copy.deepcopy(d)
    for kraj, sosedi in d.items():
        for sosed in sosedi:
            d2[sosed].add(kraj)
    return d2
...
def veljavna_pot(pot, povezave):
    if not pot:
        return True
    prejsni_element = None
    isti_element = None
    for element in pot:
        if prejsni_element:
            if element not in povezave[prejsni_element]:
                return False
            if element == isti_element:
                return False
        isti_element = prejsni_element
        prejsni_element = element
    return True


def izberi(izbor, prepovedan):
    izbran = random.choice(list(izbor))
    if izbran == prepovedan:
        izbran = izberi(izbor, prepovedan)
    return izbran
...
def potnik(zacetek, korakov, povezave):
    seznam = []
    while korakov >= 0:
        if veljavna_pot(seznam + [zacetek], povezave):
            seznam.append(zacetek)
            korakov -= 1
        zacetek = izberi(povezave[zacetek], None)
    return seznam
...
def prestej(s):
    slovar = {}
    for element in s:
        slovar[element] = s.count(element)
    return slovar


def pristej_slovar(s, t):
    for kljuc, element in t.items():
        if not s.get(kljuc, None):
            s[kljuc] = 0
        s[kljuc] += element
...
def pomembnost(potnikov, korakov, povezave, normaliziraj):
    ...
    for i in range(0, potnikov):
        zacetek = random.choice(list(povezave))
        pot = potnik(zacetek, korakov, povezave)
        pristej_slovar(skupna_pot, prestej(pot))
    if normaliziraj:
        st_krajev = sum(skupna_pot.values())
        for mesto, stevilo in skupna_pot.items():
            skupna_pot[mesto] = stevilo / st_krajev
    return skupna_pot
...
def razsiri(kraji, povezave):
    mnozica_sosedov = set()
    for kraj in kraji:
        mnozica_sosedov = mnozica_sosedov.union(dvosmerno(povezave)[kraj])
    return mnozica_sosedov - kraji
...
def razdalja(kraj1, kraj2, povezave):
    ...
    while True:
        stevec += 1
        nove_povezave2 = set()
        for kraj in vse_povezave:
            nove_povezave = razsiri({kraj}, povezave)
            if kraj2 in nove_povezave:
                return stevec
            nove_povezave2 = nove_povezave2.union(nove_povezave)
        vse_povezave = nove_povezave2


def razsiri2(kraji, povezave):
    slovar_sosedov = {}
    for kraj in kraji:
        mnozica_dvosmerno = dvosmerno(povezave)[kraj]
        for element in mnozica_dvosmerno:
            slovar_sosedov[element] = kraj
    return slovar_sosedov


def najkrajsa_pot(kraj1, kraj2, povezave):
    obiskani_kraji = set()
    preglej_poti = []
    preglej_poti.append([kraj1])

    if kraj1 == kraj2:
        return []

    while preglej_poti:
        trenutna_pot = preglej_poti.pop(0)
        trenutni_kraj = trenutna_pot[-1]
        if trenutni_kraj not in obiskani_kraji:
            sosedni_kraji = razsiri({trenutni_kraj}, povezave)
            for sosedni_kraj in sosedni_kraji:
                nova_pot = trenutna_pot.copy()
                nova_pot.append(sosedni_kraj)
                preglej_poti.append(nova_pot)
                if sosedni_kraj == kraj2:
                    return nova_pot

            obiskani_kraji.add(trenutni_kraj)
