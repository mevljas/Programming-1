...
def razbij(n):
    ...
    while n > 0:
        ...
        seznam_potenc.append(potenca)
        n -= potenca
    return seznam_potenc
...
def razlika(s, t):
    seznam_razlik = []
    for element in s:
        if not vsebuje(t, element):
            seznam_razlik.append(element)
    for element in t:
        if not vsebuje(s, element):
            seznam_razlik.append(element)
...
def razbij_seznam(s):
    razbit_seznam = []
    for element in s:
        razbit_seznam.append(razbij(element))
    return razbit_seznam
...
def zdruzi_sezname(s):
    zdruzen_seznam = []
    for seznam in s:
        zdruzen_seznam = razlika(zdruzen_seznam, seznam)
    return zdruzen_seznam

def poteza_racunalnik(s):
    razbit_seznam = razbij_seznam(s)
    zdruzen_seznam = zdruzi_sezname(razbit_seznam)
    if zdruzen_seznam == []:
        vrstica = randint(0, len(s) - 1)
        obstojeci_kovanci = s[vrstica]
        kovanci = randint(1, obstojeci_kovanci)
        return vrstica, kovanci

    else:
        for vrstica in range(len(s)):
            razlika_vrstice = razlika(razbij(s[vrstica]), zdruzen_seznam)
            vsota_razlike = vsota(razlika_vrstice)
            if s[vrstica] >= vsota_razlike:
                return vrstica, (s[vrstica] - vsota_razlike)
        else:
            vrstica = randint(0, len(s) - 1)
            obstojeci_kovanci = s[vrstica]
            kovanci = randint(1, obstojeci_kovanci)
